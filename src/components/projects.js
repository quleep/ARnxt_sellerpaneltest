...
function Projects() {
        ...
        if (typeof sessionStorage.getItem('token') === "string") {
            if (true) {
                axios.get(`${url}/user/profile/${token}`).then(resp => {
                    if (resp.data.user) {
                        axios.post(`${url}/user/projects/${token}`).then(response => {
                            setUser(true);
            ...
            if (create) {
                axios.post(`${url}/user/user_project?token=${token}&name=${name}`)
                    .then(resp => {
                        if (resp.data.status) {
                            axios.post(`${url}/user/projects/${token}`)
                                .then(response => {
    ...
    const deleteHandle = (user, proj) => {
        axios.post(`${url}/user/delete_project?userId=${user}&projectId=${proj}`)
            .then(resp => {
                if (resp.data.error) {
                    if (errorName.length > 0) {
                        setErrorName('');
                    ...
                    let id = projectsList.findIndex(project => project.projectId === proj && project.userId === user);
        ...
        if (user === true) {
            return (
                <div className='container userProCont'>
                    ...
                    {success === true &&<div role="alert" className='userPro__alert'>Project added</div>}
                    {error.length > 0 && <div role="alert" className='userPro__alert'>{error}</div>}
                    {imgSuccess.length > 0 && <div role="alert" className='userPro__alert'>{imgSuccess}</div>}
                    {deleteMsg.length > 0 &&  <div role="alert" className='userPro__alert'>{deleteMsg}</div>}
...

                    {
                        projectsList.length > 0 ?
                                ...
                                return (
                                        ...
                                        <div className='userPro__project'>
                                            <div className="userPro__project__upper">
                                                <Link to={`/user/project/${projectId}/${userId}`}
                                                      className='userPro__link'>
                                                {projectName}<span className='linksym'>&#128279;</span>
                                                </Link>
                                                <button onClick={() => deleteHandle(userId, projectId)}
                            ...
                            :
                            <h4 className='userPro__noProHeading'>There are no projects available</h4>}
                    ...
                    <div className='userPro__formmDiv'>
                        ...
                        <form className='userPro__form' onSubmit={(e) => submitHandle(e)}>
                                ...
                                {errorName.length > 0 && <div className='userPro__error'>{errorName}</div>}

                               <button className='userPro__submitBtn' type="submit">Create</button>

                                {
                                    ...
                                    <div style={{textAlign: "center", position: 'fixed',
                                        ...
                                        <h3 style={{ fontFamily:'inherit',fontWeight:'300',margin:"1rem" }}>
                                        Creating
                                        </h3>
                                        <Loader
                                            type="Circles"
