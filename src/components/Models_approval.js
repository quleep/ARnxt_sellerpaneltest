...
function Models_approval() {
  ...
  useEffect(() => {
    ...
    let limit = 40;
    axios
      .get(`${url}/admin/hiddenmodels/${token}/${limit}/${index}`)
      .then((response) => {
  ...
  const approve = (modelId, userId) => {
    ...
    if (approvedModels === 10) {
      ...
      typeof approvedModels.find((model) => model.modelId === modelId) ===
      "object"
    ) {
  ...
  const confirmApprove = () => {
    let token = sessionStorage.getItem("token");
    if (approvedModels.length <= 0) alert("No models selected");
    else
      axios
        .post(`${url}/admin/approvemodels`, {
          token: token,
          approvemodels: approvedModels,
        })
        .then((res) => {
