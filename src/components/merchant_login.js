...
class Merchant_Login extends Component {
    ...
    SubmitHandle = (e) => {
        e.preventDefault();
        axios.post(`${url}/merchant/login`, this.state).then(resp => {
            if (resp.data.status) {
    ...
    componentDidMount() {
        if (typeof sessionStorage.getItem('token') === "string") {
            const token = sessionStorage.getItem('token');
            axios.get(`${url}/merchant/profile/${token}`).then(resp => {
                if (resp.data.user) {
                    this.setState({
                        user: true
                    })
                    ...
                    <form onSubmit={this.SubmitHandle} className="form">
                        ...
                        <div className='inputDiv'>
                            ...
                            <div className='checkboxDiv'>
                                <input className="checkbox" id='checkbox' type="checkbox" onClick={this.show}/>
                                <label className="showPass" for="checkbox">Show Password</label>
                            </div>
