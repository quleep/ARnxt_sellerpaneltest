...
function ProjectFolderDrop({
    ...
    useEffect(() => {
        if (uploader) {
            ...
            axios.post(`${url}/user/project/img/upload?userId=${userId}&projectId=${projectId}`, formData, {
                headers: {
        ...
        if (acceptedFiles.length > 0) {
            ...
            let newAcceptedFiles = acceptedFiles.map(file => URL.createObjectURL(file));
    ...
    return (
        <div >
            <div style={{ display: "flex", padding: " 0 2rem", paddingBottom: '1rem' }}>
                <input id={checkBoxUniqueId} className='addUserProject__checkbox' type="checkbox" onChange={(e) => checkDropImage(e)} style={{ marginRight: "10px" }} />
                <label for={checkBoxUniqueId} className='addUserProject'>Add Images</label>
            </div>
            {dropImages && <div style={{ marginTop: "20px" }}>
                ...
                <div style={{ textAlign: "left", padding: '0 2rem', paddingBottom: '1rem' }}>
                    <button onClick={() => uploadImg()}
                        className='userPro__uploadBtn'>
                        Upload
                    </button>
                </div>
                {
