                        ...
                        if (resp.data.admin) {
                            ...
                            if (!resp.data.uploadLimit) {
                                ...
                                try {
                                    ...
                                    const response = await axios.get(`${url}/razorpay/plan/${id}`);
                                    if (response) {
                                        this.setState({
                                            user: false,
                                            merchantInfo: resp.data,
                ...
                {merchantInfo ? (
                    <div className="merchProfile__card">
                      ...
                      <h1>
                        ...
                        <span className='merchProfile__detail merchProfile__name'>{merchantInfo.name}</span>
                      </h1>

                      <p className="title">
                        <i class="fas fa-envelope-open-text"></i>
                        <span className='merchProfile__detail'>{merchantInfo.email}</span>
                      </p>

                      <p className="title">
                         <i class="fas fa-building"></i>
                        <span className='merchProfile__detail'>{merchantInfo.company} </span>
                      </p>

                      <p className="title">
                        <i class="fas fa-phone"></i>
                        <span className='merchProfile__detail'>{merchantInfo.phone}</span>
                      </p>
                        ...
                        {planDetails ? (
                          <div>
                            ...
                            <span className='merchProfile__detail'>Plan : {planDetails.name}</span>
                            </p>

                            <p className="title2">
                              <i class="fas fa-rupee-sign"></i>
                              <span className='merchProfile__detail'>Subscription Amount: {`${planDetails.amount / 100} ${
                                  planDetails.currency}/month`}
                              </span>
                            </p>

                            <p className="title2">
                              <i class="fas fa-cube"></i>
                              <span className='merchProfile__detail'>Model limit: <span className="title" ></span>{merchantInfo.uploadLimit}</span>
                            </p>
