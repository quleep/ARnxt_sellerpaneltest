...
const Analytics = () => {
  ...
  const fetchanalyticsdata =
    "https://ymxx21tb7l.execute-api.ap-south-1.amazonaws.com/production/getanalyticsdatavisualiser";
  ...
  function calculatePatternCounts(data) {
    ...
    const resultArray = Object.entries(patternCounts).map(
      ([patternno, views]) => ({
        patternno: patternno,
        views,
      })
  ...
  const handledaterangeclick = () => {
    document.querySelector(".analyticscalanderdivinside").style.display =
      "flex";
    document.querySelector(".analyticscalanderdivinsidesingle").style.display =
      "none";
    document.querySelector(".downloadcsvbutton").style.display = "none";
  };
    ...
    },
    {
      ...
      content: (
        <div className="analyticsdataoptions">
          <ul>
            <li>
              <a onClick={() => handlelastsevendays("lastday")}>
                Last day at this time
              </a>
            </li>
            <li>
              <a onClick={() => handlelastsevendays("lastsevenday")}>
                Last seven days
              </a>
            </li>
            <li>
              <a onClick={() => handlelastsevendays("lastmonth")}>Last month</a>
            </li>
            <li>
              <a onClick={handleSingleDateClick}>Single day</a>
            </li>
  ...
  function countPatternNos(array) {
    ...
    for (const date in groupedData) {
      const views = Object.entries(groupedData[date]).map(
        ([patternno, count]) => ({
          patternno,
          views: count,
        })
  ...
  newcount &&
    ...
    <td  className = 'rowanalyticsdata'>${item.views
      .map(
        (view) =>
          view.patternno +
          " -> " +
          `<span style="color: red" >  ${view.views}</span> &nbsp;`
      )
      .join(",")}</td>
    <td  className = 'rowanalyticsdata'>${item.views.reduce(
      (acc, obj) => acc + obj.views,
      0
    )}</td>
  `;
